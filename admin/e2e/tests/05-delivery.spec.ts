...
test.describe('Delivery (出貨管理)', () => {
  ...
  test('5.1 載入後顯示 Delivery Management', async ({ authedPage }) => {
    await expect(authedPage.getByText('Loading delivery')).not.toBeVisible()
    await expect(authedPage.getByText('Delivery Management')).toBeVisible()
  })
  ...
  test('5.4 搜尋欄篩選', async ({ authedPage }) => {
    await authedPage.getByPlaceholder('Search...').fill('__nonexist__')
  ...
  test('5.8 Ship 按鈕 → Dialog 顯示', async ({ authedPage }) => {
    ...
    const shipBtn = authedPage.locator('button:has-text("Ship")').first()
    if (await shipBtn.isVisible()) {
      await shipBtn.click()
      await expect(authedPage.getByText('Confirm shipment?')).toBeVisible()
  ...
  test('5.9 Delivered 按鈕 → Dialog 顯示', async ({ authedPage }) => {
    ...
    const deliverBtn = authedPage.locator('button:has-text("Delivered")').first()
    if (await deliverBtn.isVisible()) {
      await deliverBtn.click()
      await expect(authedPage.getByText('Confirm delivery?')).toBeVisible()
      await authedPage.getByRole('button', { name: '取消' }).click()
    }
  })
  ...
  test('5.10 Driver 下拉選擇', async ({ authedPage }) => {
    ...
    const driverSelect = authedPage.locator('select:has(option:has-text("Driver"))').first()
    if (await driverSelect.isVisible()) {
      await driverSelect.selectOption('Driver A')
      expect(await driverSelect.inputValue()).toBe('Driver A')
    }
  })
  ...
  test('5.12 Print 按鈕：未選取→disabled', async ({ authedPage }) => {
    const printBtn = authedPage.locator('button:has-text("Print (0)")')
  ...
  test('5.13 Preview 按鈕切換', async ({ authedPage }) => {
    ...
    const previewBtn = authedPage.locator('button:has-text("Preview")').first()
    if (await previewBtn.isVisible()) {
      await previewBtn.click()
      await expect(authedPage.locator('button:has-text("Close")').first()).toBeVisible()
    }
  })
